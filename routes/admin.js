...
// para verificar se o usuario é admin
...
router.post("/postagens/nova", eAdmin, (req, res) => {
    ...
    if (erros.length > 0) res.render("admin/addpostagem", { erros: erros });
    else {
        const novaPostagem = {
        ...
        .then((postagem) => {
            ...
            postagem
                ...
                .catch((e) => {
                    req.flash(
                        "error_msg",
                        "Houve um erro ao salvar a edição" + e
                    );
                    res.redirect("/admin/postagens");
                });
        })
        .catch((e) => {
            req.flash("error_msg", "Houve um erro ao salvar a edição" + e);
            res.redirect("/admin/postagens");
        });
});
...
router.get("/postagens/deletar/:id", eAdmin, (req, res) => {
    Postagem.remove({_id: req.params.id})
        ...
        .catch((e) => {
            req.flash(
                "error_msg",
                "Houve um erro ao deletar a postagem"
            );
            res.redirect("/admin/postagens");
        });
})
